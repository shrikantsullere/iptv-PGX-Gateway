...
import { Wallet, Search, ArrowRightLeft, ShieldCheck, Database, Link as LinkIcon } from 'lucide-react';

export default function Wallets() {
  const [wallets] = useState([
    { name: 'Gateway Cold Storage', address: '0x8f...29d', type: 'Cold', network: 'Ethereum', balance: '$12.4M' },
    { name: 'Operational Hot Wallet 1', address: '0x3a...11f', type: 'Hot', network: 'Polygon', balance: '$850K' },
    { name: 'Operational Hot Wallet 2', address: 'TE9...k2M', type: 'Hot', network: 'Tron', balance: '$1.2M' },
  ]);
    ...
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-[#13131A] border border-cyan-500/20 rounded-2xl p-6 shadow-[0_0_30px_rgba(6,182,212,0.1)] relative overflow-hidden">
           ...
           <h3 className="text-5xl font-black text-white tracking-tight">$12,450,000.00</h3>
        </div>
        <div className="bg-[#13131A] border border-orange-500/20 rounded-2xl p-6 shadow-[0_0_30px_rgba(249,115,22,0.1)] relative overflow-hidden">
           ...
           <h3 className="text-5xl font-black text-white tracking-tight">$2,050,000.00</h3>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-gray-500 text-xs font-bold uppercase border-b border-white/5 bg-white/[0.02]">
              ...
              <th className="p-4 text-right">Balance (USD Eq)</th>
            </tr>
          </thead>
            ...
            {wallets.map((w, i) => (
              <tr key={i} className="hover:bg-white/[0.02]">
                <td className="p-4 font-bold text-white flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-green-500" /> {w.name}
                </td>
                <td className="p-4 text-gray-500 font-mono text-xs">{w.address}</td>
                <td className="p-4">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${w.type === 'Cold' ? 'bg-cyan-500/10 text-cyan-500' : 'bg-orange-500/10 text-orange-500'}`}>
                    {w.type}
                  </span>
                </td>
                <td className="p-4 text-gray-400 flex items-center gap-1"><LinkIcon className="w-3 h-3"/> {w.network}</td>
                <td className="p-4 text-right font-black text-white">{w.balance}</td>
              </tr>
            ))}
          </tbody>
        </table>
